...
if __name__ == '__main__':
    ...
    rerun = False
    size_list = [15, 30, 60, 100, 150, 200, 205, 210, 220, 230, 250, 300,
                 500]
    seed_list = [276]
    if rerun:
    ...
    for i in range(len(fancy_cost_avg)):
        ...
        if rndom_cost_avg[i] == 0:
            rndom_cost_avg[i] = np.nan
        if greedy_cost_avg[i] == 0:
            greedy_cost_avg[i] = np.nan
        if bb_cost_avg == 0:
            bb_cost_avg[i] = np.nan
        if fancy_cost_avg == 0:
            fancy_cost_avg[i] = np.nan
